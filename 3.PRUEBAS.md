...
### APUNTES TEMA 3 | JOSÉ FÍLTER

### PRUEBAS
- __FORMAS DE LAS PRUEBAS.__
   -__Pruebas dinámicas__: Requieren la ejecucicón de la aplicación.
   -__Pruebas estáticas__: Se realizan sin ejecutar el código de la aplicación. La aplicacón está parada. Se analiza el código.
   
- __ESTRATEGIAS DE PRUEBA.__
   -__Caja nergra__: Se estudia el sistema desde fuera (como funciona).
   -__Caja blanca__: Se examina el codigo fuente y su ejecuación (como se estructura).

- __ESTRATEGIAS DE PRUEBA DE LA CAJA NEGRA.__
   -Se estudia el sistema desde fuera.
   -Se proporciona valores y se estudia si son correctos
   -Tiene dos técnicas: Particiones de equivalencia (Ejemplo: Muestra de calidad de las aceitunas cuando las entregas) y Valores límite (Ejemplo: El valor limite que tiene un int o un byte).

- __ESTRATEGIAS DE PRUEBA DE CAJA BLANCA.__
    -Se examina el código fuente y su estructura.
    -Se estudia su flujo
    -Tiene dos técnicas: Cobertura de código (La cobertura te dice por donde pasa el flujo.{Ejemplo: Si pasa por el if o no }) y Prueba de bucles(Ejemplo: Comprueba si entra o no el el bucle y si sale o no).
